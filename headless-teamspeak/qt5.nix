{ qt55, lib, libpulseaudio, openssl, sqlite, icu, fontconfig, freetype
, glib, udev, dbus, libxml2, libxslt, pcre, zlib, libjpeg, libpng, libtiff
}:

let version = (builtins.parseDrvName qt55.qtbase.name).version;
    overrideQt = qt55.qtbase.overrideDerivation;
in {
  qtbase = lib.setName "headless-teamspeak-qt5-${version}" (overrideQt (old: {
