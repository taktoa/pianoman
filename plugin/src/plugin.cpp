...
int ts3plugin_onClientPokeEvent(uint64 schandlerID,
    ...
    snprintf(buffer,
             512,
             "PLUGIN onClientPokeEvent: %llu %d %s %s %d",
             (long long unsigned int) schandlerID,
             fromClientID,
             pokerName,
             message,
    ...
    if(ts3Functions.getClientID(schandlerID, &myID) !=
       ERROR_ok) { /* Get own client ID */
        ts3Functions.logMessage("Error querying own client id",
                                LogLevel_ERROR,
                                "Plugin",
                                schandlerID);
        return 0;
    ...
    if(fromClientID != myID) { /* Don't reply when source is own client */
        if(ts3Functions.requestSendPrivateTextMsg(
               schandlerID, "Received your poke!", fromClientID, NULL) !=
           ERROR_ok) {
            ts3Functions.logMessage("Error requesting send text message",
                                    LogLevel_ERROR,
                                    "Plugin",
                                    schandlerID);
        }
...
void ts3plugin_onClientServerQueryLoginPasswordEvent(
    uint64 schandlerID, const char *loginPassword) {}
...
void ts3plugin_onMenuItemEvent(uint64 schandlerID,
    ...
    printf("PLUGIN: onMenuItemEvent: schandlerID=%llu, type=%d, menuItemID=%d, "
           "selectedItemID=%llu\n",
           (long long unsigned int) schandlerID,
           type,
           menuItemID,
    ...
    case PLUGIN_MENU_TYPE_GLOBAL:
        /* Global menu item was triggered. selectedItemID is unused and set to
         * zero. */
        switch(menuItemID) {
    ...
    case PLUGIN_MENU_TYPE_CHANNEL:
        /* Channel contextmenu item was triggered. selectedItemID is the
         * channelID of the selected channel */
        switch(menuItemID) {
    ...
    case PLUGIN_MENU_TYPE_CLIENT:
        /* Client contextmenu item was triggered. selectedItemID is the clientID
