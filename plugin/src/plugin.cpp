...
void pluginLogDebug(std::string message) { pluginLog("DEBUG", message); }

//! Call pluginLog with level INFO.
void pluginLogInfo(std::string message) { pluginLog("INFO", message); }

//! Call pluginLog with level WARNING.
void pluginLogWarning(std::string message) { pluginLog("WARNING", message); }

//! Call pluginLog with level ERROR.
void pluginLogError(std::string message) { pluginLog("ERROR", message); }
...
int ts3plugin_init() {
    pluginLogInfo("Initializing server");
    rpc_server = new rpc::server_handle_t;
    pluginLogInfo("Starting server");
...
void ts3plugin_shutdown() {
    pluginLogInfo("Shutting down");
    pluginLogDebug("Starting shutdown process");

    pluginLogDebug("Checking if RPC server exists");
    if(rpc_server) {
        pluginLogDebug("RPC server found");
        pluginLogDebug("Sending shutdown message");
        Json::Value root;
        root["event"] = "shutdown";
        rpc_server->send_event(root);
        pluginLogDebug("Telling RPC server to shut down");
    ...
    pluginLogDebug("Plugin shutdown completed");
}
